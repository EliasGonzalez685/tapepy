-- Extiende estado_cuota_plataforma_organizacion con identificación
-- (cédula) y el estado de bloqueo manual (de la cuenta y de su
-- parada), para que el dueño de plataforma vea todo -- quién pagó, con
-- qué método, quién es, y si está bloqueado -- en la misma pantalla de
-- Cuotas de plataforma (pedido de Elias 2026-09-07).
drop function if exists public.estado_cuota_plataforma_organizacion(uuid);
...
returns table(
  ...
  bloqueo_manual_hasta timestamptz,
  bloqueo_manual_motivo text,
  parada_bloqueada boolean,
  parada_bloqueada_hasta timestamptz,
  parada_bloqueada_motivo text
)
language plpgsql
stable
security definer
...
begin
  ...
  select
    ...
    cp.id,
    u.bloqueo_manual,
    u.bloqueo_manual_hasta,
    u.bloqueo_manual_motivo,
    coalesce(pc.bloqueada, pp.bloqueada, false),
    coalesce(pc.bloqueada_hasta, pp.bloqueada_hasta),
    coalesce(pc.bloqueada_motivo, pp.bloqueada_motivo)
